...
| プラグイン | 対象 | 中身 |
|---|---|---|
| `sc2-skills` | 利用者 | 基本2スキル: `sc2-memory-search`（記憶の検索）・`sc2-memory-record`（記憶の記録）／応用2スキル: `sc2-news-pick`（ニュース収集）・`sc2-meeting-log`（会議メモ記録） |
| `sc2-ops` | 導入・管理担当 | `sc2-prompt-retrofit` — 既存のシステムプロンプト・CLAUDE.md・AGENTS.md を渡すと、新規/利用中を判定して SynapseConnect を最適に使える形へ修正 |
...
2. 基本ルール（システムプロンプト）を設定済みであること。基本ルールが「いつ動くか」を決め、
   ...
   `plugins/sc2-ops/skills/sc2-prompt-retrofit/references/base-rules.md` にあります。
...
- **Claude Code**:
  ...
  /plugin install sc2-skills@synapseconnect-skills
  ```
  （導入・管理担当は `sc2-ops@synapseconnect-skills` も）
- **Claude.ai（チャット）/ Claude Desktop**: `plugins/sc2-skills/skills/` 配下の各スキルフォルダを
